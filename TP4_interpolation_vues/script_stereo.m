% TDM Stereovision
clear; close all; clc; dbstop if error; path(pathdef);

%% Estimation de disparite

% Charge les images

I1 = imread('data/tsukuba2.png');  %left image
I2 = imread('data/tsukuba4.png');  %right image
I1 = double(I1)/255;
I2 = double(I2)/255;
...
linkaxes([h1,h2,h3]); %zoom synchronisÈ

%% Calcul de la disparite

maxs = 35;%disparite maximum
win_size = 16; % taille fenetre carrÈ ou est calculÈ disparite

D1 = estimate_disparity(I1,I2, 0, maxs, win_size); %disparitÈ I1 vers I2
D2 = estimate_disparity(I2,I1, -maxs, 0, win_size); %disparitÈ I2 vers I1

%DisparitÈ = cmb de pixels je me suis dÈcalÈ en x (
%Disparite = inversement proportionnel a la profondeur
%Disparite renvoye = disparite pour laquelle voisinnage pixel (i1,j1)
% et voisinage du pixel (i1,j1-D1(i1,j1)) correspondant dans I2
%sont le plus similaire
% i = ligne = y = CTE ICI  ;   j = colonne = x
%I(i,j)=I(y,x)
...
%% Prediction de I1 a partir de I2 %Bien car peut remonter passe
...
%% Prediction de I2 a partir de I1 %DÈgueu car peut pas prÈdire le futur
...
Im = double(imread('data/tsukuba3.png'))/255;  %middle image
a = 0.5;
...
for i=1:length(a_list)
    ...
    Imp1 = interp2color(I1, X1_m,Y);
    Imp2 = interp2color(I2, X2_m,Y);
    Imp = 0.5*Imp1 + 0.5*Imp2; % Bien choisir la ponderation de moyennage
